...
public class ModelView extends Layout{
    ...
    public ModelView(Model model) {
        ...
        for (Brand brand : this.brandManager.findAll()) { //model ekle kısmında markaları dolaşıyor
            this.cmb_brand.addItem(new ComboItem(brand.getId(), brand.getName()));
        }
        this.cmb_model_fuel.setModel(new DefaultComboBoxModel<>(Model.Fuel.values())); //model ekle penceresinde yakıt seçenekleri çıkar
        this.cmb_model_gear.setModel(new DefaultComboBoxModel<>(Model.Gear.values())); //model ekle penceresinde vites türleri çıkar
