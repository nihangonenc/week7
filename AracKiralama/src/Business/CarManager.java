...
public class CarManager {
    ...
    public boolean update(Car car) {
        if (this.getById(car.getId()) == null) {
            Helper.showMsg(car.getId() + "ID kaydı bulunamadı");
            return false;
        }
    ...
    public ArrayList<Car> searchForBooking(String strt_date, String fnsh_date, Model.Type type, Model.Gear gear, Model.Fuel fuel) {
        //belirli tarih aralığında müsait olan araçları bulma
        String query = "SELECT * FROM public.car as c LEFT JOIN public.model as m"; //left joinle tablo birleştirme
        ...
        ArrayList<Car> searchedCarList = this.carDao.selectByQuery(query); //özel query oluşturma
